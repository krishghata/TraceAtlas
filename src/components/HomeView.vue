...
<div style="flex:1;overflow-y:auto;min-height:0;height:100%;padding:24px 24px;box-sizing:border-box">

  <!-- ── Header ──────────────────────────────────────────────────────────── -->
  <div style="text-align:center;margin-bottom:24px">
    <img src="/logo-light.svg" alt="TraceAtlas" style="width:48px;height:48px;margin-bottom:10px"/>
    <h1 style="margin:0 0 5px;font-size:20px;font-weight:700;color:#e2e8f0">TraceAtlas</h1>
    <p style="margin:0;font-size:12px;color:#475569">Select a tool to open it in a new tab</p>
  </div>

  <!-- ── Feature grid ────────────────────────────────────────────────────── -->
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:10px;max-width:900px;margin:0 auto">
    <div v-for="f in features" :key="f.type"
      @click="$emit('open', f.type)"
      :style="`padding:16px 16px;border-radius:10px;border:1px solid ${f.open ? '#38bdf8' : '#1e3a5f'};
      ...
      <!-- Icon -->
      <div style="font-size:26px;line-height:1">{{ f.icon }}</div>
  ...
  <!-- ── Footer ──────────────────────────────────────────────────────────── -->
  <div style="max-width:900px;margin:24px auto 0;display:flex;justify-content:space-between;align-items:center;padding-bottom:12px;flex-wrap:wrap;gap:8px">
