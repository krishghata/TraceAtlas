  ...
  <div style="flex:1;background:#0d1b2a;border-radius:8px;border:1px solid #1e3a5f;overflow:hidden;position:relative;min-height:120px">
    ...
    <svg v-else :viewBox="`0 0 ${SVG_W} ${SVG_H}`" preserveAspectRatio="none"
      ...
      <g v-for="g in gridLines" :key="g.label">
        <line :x1="PL" :y1="g.y" :x2="SVG_W - PR" :y2="g.y"
          stroke="#1e3a5f" stroke-width="0.8"/>
        <text :x="PL - 5" :y="g.y + 3.5"
          text-anchor="end" fill="#334155" font-size="9" font-family="monospace">
          {{ g.label }}
        </text>
      </g>

      <!-- X-axis seq labels every 10 packets -->
      <text v-for="xl in xLabels" :key="xl.seq"
        :x="xl.cx" :y="SVG_H - 2"
        text-anchor="middle" fill="#334155" font-size="9" font-family="monospace">
        {{ xl.seq }}
      </text>

      <!-- Dropped packet × markers -->
      <g v-for="pt in droppedPoints" :key="'x' + pt.seq">
        <line :x1="pt.cx - 4" :y1="SVG_H - PB - 10" :x2="pt.cx + 4" :y2="SVG_H - PB"
          stroke="#ef4444" stroke-width="1.5"/>
        <line :x1="pt.cx + 4" :y1="SVG_H - PB - 10" :x2="pt.cx - 4" :y2="SVG_H - PB"
          stroke="#ef4444" stroke-width="1.5"/>
      </g>

      <!-- Area fill under the RTT line -->
      <path v-if="areaPath" :d="areaPath"
        fill="rgba(56,189,248,0.06)"/>

      <!-- RTT line — breaks on dropped packets -->
      ...
      <circle v-for="pt in validPoints" :key="'dot' + pt.seq"
        :cx="pt.cx" :cy="pt.cy" r="2.8"
        :fill="pt.color" :style="`filter:drop-shadow(0 0 3px ${pt.color})`"/>
    </svg>
  </div>

  <!-- ── Legend ──────────────────────────────────────────────────────────── -->
  <div style="display:flex;gap:18px;flex-shrink:0;font-size:11px;color:#475569;padding:0 2px">
    <span><span style="color:#22c55e">●</span>  &lt;50ms — Good</span>
    <span><span style="color:#f59e0b">●</span>  &lt;150ms — Fair</span>
    <span><span style="color:#f97316">●</span>  &lt;300ms — Poor</span>
    <span><span style="color:#ef4444">●</span>  &gt;300ms / Timeout</span>
  </div>
